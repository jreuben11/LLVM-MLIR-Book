...
**Out:** Implementation of a proof assistant (Coq/Lean/Isabelle internals are referenced, not derived); model theory and full mathematical logic foundations beyond what's needed to use them; verified hardware (CHERI, seL4 hardware extensions); pure-mathematics commutative algebra beyond what polyhedral and type-theory chapters need.
...
## Part XXVI — Ecosystem and Frontiers *(~54 pages, 4 ch)*

| # | Chapter | Topics |
|---|---------|--------|
| 177 | rustc and the Rust Codegen Backend | rustc architecture: HIR → MIR → codegen; MIR as Rust's SSA-like IR; borrow-checker impact on codegen; `rustc_codegen_llvm`: MIR → LLVM IR lowering; ADT/enum/trait-object type lowering; panics, unwinding, and personality functions; the Cranelift alternative backend (`rustc_codegen_cranelift`); the GCC backend (`rustc_codegen_gcc`); Rust-specific LLVM attributes (`nounwind`, `noalias`, `dereferenceable`); LTO in Rust (`-C lto=thin/fat`); PGO; cross-compilation and target triples; rustc's vendored LLVM fork |
| 178 | LLVM/MLIR for AI: The Full Stack | The AI compilation hierarchy: framework eager → traced/torchscript/dynamo → StableHLO/HLO → MLIR dialects → target ISA; key dialects: `tosa`, `stablehlo`, `tensor`, `linalg`, `vector`, `memref`, `gpu`; quantization: `quant` dialect, INT8/INT4/FP8 lowering paths; dynamic shapes and StableHLO dynamism; hardware targets: CUDA, ROCm, NPUs (Apple ANE, Hexagon DSP, Edge TPU); the inference deployment stack (IREE, TFLite, ONNX Runtime, TensorRT); training vs inference compilation differences; FlashAttention and kernel fusion in the MLIR stack; synthesis across Parts XIX–XXIII |
| 179 | AI-Guided Compilation | MLGO: the ML-guided optimization framework; the RL-trained inliner (features, reward, training loop — cross-ref Ch66); ML-based register allocator eviction; the TFLite runtime embedded in LLVM; offline training and online serving model; ML-based cost models for instruction scheduling; Ansor/AutoTVM/Halide autotuner architecture; IREE tuning infrastructure; Triton's autotuner; profile inference with ML; LLM-assisted compiler development: TableGen authoring, pass optimization, LLM-guided fuzzing; LLVM IR as training data for code LLMs; neural superoptimization; learned middle-end optimization policies |
| 180 | ANTLR4, TreeSitter, and Language Tooling with LLVM | The tooling-vs-compilation distinction: when to use each; ANTLR4: LL(*) grammar syntax, visitor/listener patterns, the ANTLR4 C++ runtime; connecting an ANTLR4 parse tree to LLVM IR generation; error recovery in ANTLR4; TreeSitter: incremental GLR parsing for editors; grammar authoring (`.js` grammars); the C API and Rust/Python bindings; why TreeSitter is unsuitable for compilation (ambiguity, partial parses); using TreeSitter for syntax highlighting, code navigation, and lightweight semantic analysis; nvim-treesitter and LSP integration; the two-tier frontend pattern: TreeSitter for tooling + ANTLR4/hand-written for compilation; comparison matrix: hand-written (Clang), ANTLR4, TreeSitter, PEG (pest/nom), parser combinators |
...
| XXVI | Ecosystem and Frontiers | 4 | 54 |
| | Appendices A–H | 8 | 95 |
| | **Total** | **180 chapters + 8 appendices** | **~2249 pages** |
...
- **Vol 4 — Verified Compilation and Engineering Practice** (XXIV–XXVI + Appendices): ~359 pages

So really **five volumes** at this size:
- Vol 1: Theory (445 pp)
- Vol 2: LLVM Core (1115 pp)
- Vol 3: Clang and Runtimes (870 pp)
- Vol 4: MLIR and ML Compilation (730 pp)
- Vol 5: Verified Compilation + Operations + Ecosystem + Appendices (359 pp)

Total across volumes: 3465 pages of *target* material — but with the cross-references and shared appendices, the consolidated single set is the ~2195 above.
