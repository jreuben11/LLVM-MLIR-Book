...
- [1. LLVM IR's Type System: A Stripped-Down, Non-Polymorphic Design](#1-llvm-irs-type-system-a-stripped-down-non-polymorphic-design)
  - [Ground Theory](#ground-theory)
  - [The Type Grammar](#the-type-grammar)
  - [Nominal vs. Structural Struct Types](#nominal-vs-structural-struct-types)
  - [Function Types](#function-types)
  - [The Opaque Pointer Migration](#the-opaque-pointer-migration)
  - [Integer Signedness: A Type-Level Absence](#integer-signedness-a-type-level-absence)
  - [Type Safety in LLVM IR: Deliberately Weak](#type-safety-in-llvm-ir-deliberately-weak)
- [2. Why Generics Monomorphize Before Reaching LLVM](#2-why-generics-monomorphize-before-reaching-llvm)
  - [The Polymorphism Erasure Imperative](#the-polymorphism-erasure-imperative)
  - [C++ Templates: Full Monomorphization](#c-templates-full-monomorphization)
  - [Rust Generics: Monomorphize by Default, Erase via `dyn Trait`](#rust-generics-monomorphize-by-default-erase-via-dyn-trait)
  - [Go Generics (1.18+): GC Shape Stenciling](#go-generics-118-gc-shape-stenciling)
  - [Haskell: Dictionary Passing Through GHC Core](#haskell-dictionary-passing-through-ghc-core)
  - [Java and the JVM: Full Type Erasure with Runtime Reification](#java-and-the-jvm-full-type-erasure-with-runtime-reification)
  - [Swift: Witness Tables as an Intermediate Position](#swift-witness-tables-as-an-intermediate-position)
  - [The Design Decision](#the-design-decision)
- [3. MLIR's Richer, Theory-Aligned Type System](#3-mlirs-richer-theory-aligned-type-system)
  - [Beyond LLVM IR's Flat Types](#beyond-llvm-irs-flat-types)
  - [Parameterized Types: tensor, memref, vector](#parameterized-types-tensor-memref-vector)
  - [Dialect-Defined Types: Extending the Universe](#dialect-defined-types-extending-the-universe)
  - [TypeStorage Uniquing](#typestorage-uniquing)
  - [Type Interfaces: Ad-Hoc Polymorphism](#type-interfaces-ad-hoc-polymorphism)
  - [Op Type Constraints: Verification as a Type-Level Property](#op-type-constraints-verification-as-a-type-level-property)
  - [MLIR vs. LLVM IR: A Type-Theoretic Comparison](#mlir-vs-llvm-ir-a-type-theoretic-comparison)
- [4. Clang's C/C++ Type Representation: QualType, Sugar, Dependent Types](#4-clangs-cc-type-representation-qualtype-sugar-dependent-types)
  - [The QualType / Type* Duality](#the-qualtype-type-duality)
  - [Canonical Types and the Sugar Taxonomy](#canonical-types-and-the-sugar-taxonomy)
  - [ASTContext as the Type Factory](#astcontext-as-the-type-factory)
  - [Built-in Types and Target Sensitivity](#built-in-types-and-target-sensitivity)
  - [Dependent Types in Templates](#dependent-types-in-templates)
  - [The Canonical Type Check for Template Specialization](#the-canonical-type-check-for-template-specialization)
  - [Type Errors in Template Instantiation](#type-errors-in-template-instantiation)
- [5. Rust's Borrow Checker as a Substructural Type System](#5-rusts-borrow-checker-as-a-substructural-type-system)
  - [MIR: The IR Where Borrowing Lives](#mir-the-ir-where-borrowing-lives)
  - [Constraint Generation and Solving](#constraint-generation-and-solving)
  - [The Affine Type Discipline in Practice](#the-affine-type-discipline-in-practice)
  - [Two-Phase Borrows and the Borrow Checker's Edge Cases](#two-phase-borrows-and-the-borrow-checkers-edge-cases)
  - [Rust's Type-Level Guarantees and Their LLVM IR Manifestation](#rusts-type-level-guarantees-and-their-llvm-ir-manifestation)
  - [The Transition to LLVM IR](#the-transition-to-llvm-ir)
- [6. ML Compilers: From HM to LLVM via Typed Intermediates](#6-ml-compilers-from-hm-to-llvm-via-typed-intermediates)
  - [GHC's Typed Pipeline: System FC and STG](#ghcs-typed-pipeline-system-fc-and-stg)
  - [A System FC Snippet: Seeing the Coercions](#a-system-fc-snippet-seeing-the-coercions)
  - [MLton: Standard ML to Flat IR](#mlton-standard-ml-to-flat-ir)
  - [OCaml: Lambda IR and Flambda2](#ocaml-lambda-ir-and-flambda2)
  - [The Type-Erasure Gradient Across ML Compilers](#the-type-erasure-gradient-across-ml-compilers)
- [7. Alive2 and Refinement-Style Verification on LLVM IR](#7-alive2-and-refinement-style-verification-on-llvm-ir)
  - [The Problem Alive2 Solves](#the-problem-alive2-solves)
  - [Refinement as Subtyping](#refinement-as-subtyping)
  - [Poison and Undef in the Type Lattice](#poison-and-undef-in-the-type-lattice)
  - [Alive2 in Practice: A Caught Optimization Bug](#alive2-in-practice-a-caught-optimization-bug)
  - [Freeze and the Elimination of Undef](#freeze-and-the-elimination-of-undef)
  - [What "Type Safety" Means at LLVM IR Level](#what-type-safety-means-at-llvm-ir-level)
- [8. constexpr/consteval as Total Functional Programming](#8-constexprconsteval-as-total-functional-programming)
  - [The constexpr Sublanguage](#the-constexpr-sublanguage)
  - [The Evolution: C++11 → C++23](#the-evolution-c11-c23)
  - [consteval: Mandatory Compile-Time Evaluation](#consteval-mandatory-compile-time-evaluation)
  - [Template Metaprogramming as Accidental Turing Completeness](#template-metaprogramming-as-accidental-turing-completeness)
  - [constinit and the Initialization Fiasco](#constinit-and-the-initialization-fiasco)
  - [Immediate-Mode Constant Evaluation and the Clang Interpreter](#immediate-mode-constant-evaluation-and-the-clang-interpreter)
  - [Constant Propagation to LLVM IR](#constant-propagation-to-llvm-ir)
  - [The Dependent Type Connection](#the-dependent-type-connection)
...
- **Polonius 2.0 borrow checker stabilization in rustc**: The Polonius Datalog-based lifetime solver (Section 5) is tracking toward stable Rust as `#[feature(polonius)]`. The RFC landed in 2024; the remaining work is correctness edge-cases and compile-time performance. Follow progress at [rust-lang/polonius](https://github.com/rust-lang/polonius) and the `t-types` Zulip stream. Once stable, programs previously rejected by the NLL solver due to overly conservative region reasoning will compile without `unsafe`.
- **MLIR type-level shape inference via the Shape dialect and `ShapeInferenceInterface`**: Ongoing work (tracked in [LLVM Discourse #mlir](https://discourse.llvm.org/c/mlir/)) to strengthen MLIR's parameterized tensor shapes (Section 3) — propagating static shapes through more op combinations and extending the `DimOp` folder. Near-term patches target `tensor.reshape` and `linalg` ops that currently produce `?`-ranked results where static shapes are provable.
- **C++26 `constexpr` expansion — `std::string` and `std::vector` as `constexpr` from P2679/P3012**: C++26 extends the `constexpr` sublanguage (Section 8) to allow `std::string` and `std::vector` in `constexpr` contexts through the transient allocation model. Clang tracking issue [clang-project #80344](https://github.com/llvm/llvm-project/issues/80344) follows implementation progress. When complete, the gap between `constexpr` C++ and a full compile-time functional language narrows further.
- **Alive2 integration into LLVM CI for all InstCombine transforms**: The LLVM community is expanding the set of InstCombine patterns that are mechanically verified by Alive2 (Section 7) on every pull request, via the `llvm/test/Transforms/InstCombine/alive2/` directory. Current coverage is partial; the push to verify all ~2,000 patterns is active work. See LLVM RFC [discourse.llvm.org/t/rfc-mandatory-alive2-for-instcombine](https://discourse.llvm.org/t/rfc-mandatory-alive2-for-instcombine/73543).

### 2.5-Year Horizon (Mid-Term, by ~October 2028)

- **GHC CPS-based typed IR replacing STG**: GHC 9.x is developing a new CPS-based typed intermediate representation to replace STG (Section 6), designed to preserve richer type information through the code-generator and enable better LLVM IR emission. The new IR would maintain enough type information to emit precise GC root maps without the current heuristic approach. Follow [GHC Proposal #0000 — Typed CPS IR](https://github.com/ghc-proposals/ghc-proposals) and the GHC wiki page on the "next-generation backend."
- **Clang `constexpr` interpreter (P2996/reflection) as a first-class compile-time type system layer**: C++26/27 compile-time reflection (P2996, merged into the C++26 draft) introduces `std::meta::info` — a compile-time type that represents program entities. The Clang experimental interpreter (`clang/lib/AST/Interp/`) must be extended to evaluate reflection queries. This makes the `constexpr` sublanguage (Section 8) capable of inspecting types at compile time, approaching the expressiveness of a dependently-typed metaprogramming layer without full dependent types. Track [Clang P2996 tracking issue](https://github.com/llvm/llvm-project/issues/80454).
- **Vellvm 3.0 and formalized MLIR semantics in Coq**: The Vellvm project (Chapter 170) is extending its Coq formalization to cover MLIR's core dialect. Once available, the refinement relation (Section 7) for MLIR rewrites will be mechanically checkable, extending Alive2-style verification to MLIR transformations such as `linalg` tiling and `bufferization`. See [Vellvm GitHub](https://github.com/vellvm/vellvm) and ongoing PLDI and POPL submissions from the Penn/UMass group.
- **Rust's `impl Trait` and RPIT lifetime capture rules stabilizing**: The 2024 Rust edition changed lifetime capture semantics for `impl Trait` in return position (RPIT). The full `use<>` capture syntax and its interaction with the NLL borrow checker (Section 5) is expected to settle by 2027 as the community migrates to edition 2024+ in crates.io. The substructural type system's expressiveness for iterator adapters and async combinators depends directly on this stabilization.

### 5-Year Horizon (Long-Term, by ~2031)

- **Dependent types in a mainstream LLVM-targeting language**: Several languages targeting LLVM are experimenting with dependent types: Lean 4 (which already uses LLVM), Idris 2, and the research language Hazel (NSF-funded). The practical question of how to lower dependent type indices into LLVM IR — either by monomorphization (Section 2 strategy) or by passing type witnesses analogous to Go's GC shape stenciling — remains open. A production-grade dependent type system over LLVM IR would require extending the LLVM alias analysis and optimization pipeline to consume type-index information, a multi-year research program.
- **Formal proof of MLIR's type system soundness**: MLIR's extensible dialect-defined type universe (Section 3) has no published formal soundness proof. As MLIR is used for safety-critical domains (ML hardware, verified compilation in aerospace), pressure will grow for a formal treatment analogous to RustBelt [Jung et al., JACM 2021] for MLIR's type system and verifier. Candidate proof assistants are Lean 4 (for its LLVM backend alignment) and Rocq (Coq 9+). The `mlir-hs` Haskell embedding project is an early step toward a mechanized MLIR semantics.
- **LLVM IR type system extension for provenance-aware pointer types**: The current opaque-pointer model (Section 1) discards pointee-type information, replacing it with TBAA metadata. The ongoing LLVM memory model formalization work (building on Kang et al. "A Promising Semantics for Relaxed-Memory Concurrency," POPL 2017 and Memarian et al. "Exploring C Semantics and Pointer Provenance," POPL 2019) may require augmenting LLVM IR's type system with provenance annotations — typed pointers that carry allocation identity, enabling the alias analysis and formal model to agree. This would effectively re-introduce a limited form of typed pointers at the provenance level, closing the circle opened by the opaque-pointer migration.
