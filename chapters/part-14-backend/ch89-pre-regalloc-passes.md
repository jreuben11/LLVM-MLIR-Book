...
- **GlobalISel RegisterCoalescer integration**: The ongoing effort to make `RegisterCoalescer` fully cooperate with GlobalISel's `MachineIRBuilder`-produced copies is tracked in LLVM discourse (RFC: "GlobalISel coalescing quality gap"). Near-term patches target coalescing of G_COPY instructions generated by legalisation and selection without falling back to SelectionDAG.
- **Live interval sub-range accuracy for partial defs on RISC-V**: RISC-V's lack of sub-register writes (all writes are full-register) creates redundant sub-range splits in `LiveInterval::SubRange`; patches narrowing the sub-range universe for scalar RISC-V are in review (see llvm-project#87432 and related).
- **MachineCombiner extensions for matrix-multiply reassociation**: AArch64 SME2 and SVE2 outer-product sequences expose new reassociation opportunities; `TargetInstrInfo::getMachineCombinerPatterns()` is being extended for `SMOPA`/`BFMOPA` instruction chains.
- **Pre-RA scheduling latency model updates for Intel Clearwater Forest (P-core Cougar Cove)**: Updated `SchedMachineModel` records for Clearwater Forest's 64-wide SIMD units and expanded load-store pipelines are expected in the LLVM 22 release cycle.

### 2.5-Year Horizon (Mid-Term, by ~October 2028)

- **Unified pre-RA pass pipeline for SelectionDAG and GlobalISel**: The long-term GlobalISel roadmap (discussed at LLVM Developers' Meeting 2025) targets a shared `TwoAddressInstructionPass` and `PHIElimination` that operates directly on `MachineIR` post-GISel without the SSA-to-MIR impedance mismatch, removing the current dual-pipeline maintenance burden.
- **ML-guided `GenericScheduler` heuristics**: Following the success of MLGO for inlining and regalloc eviction, similar reinforcement-learning-trained heuristics for pre-RA scheduling priority functions are under research at Google and Meta; the approach would replace the hand-tuned `CritPathBonus`/`RegisterPressureDelta` weights with a learned model compiled into the scheduler at build time.
- **Improved coalescing for speculative/predicated ISAs (AArch64 SVE predicate registers)**: Sub-register coalescing across SVE predicate widths (`p0.b`, `p0.h`, `p0.s`, `p0.d`) currently produces excessive copies; a redesigned lane-mask model for variable-length predicates is planned once SVE2 support stabilises.
- **`SlotIndexes` redesign for MIR bundles**: The current `SlotIndexes` representation breaks down for VLIW targets that bundle multiple operations into a single `MachineInstr` slot. A per-operation slot refinement compatible with Hexagon and future VLIW targets is on the mid-term roadmap.

### 5-Year Horizon (Long-Term, by ~2031)

- **Probabilistic live-interval computation for speculative multi-path IR**: As compilers gain richer probabilistic profile information (from hardware branch predictors and PGO), live intervals may be extended to carry probability-weighted liveness — allowing the register allocator to treat "hot path live, cold path dead" registers differently from uniformly live registers, reducing spills on hot paths.
- **Formal verification of `PHIElimination` and `TwoAddressInstructionPass` correctness**: Following CompCert's verified liveness analysis and Alive2's IR-level verification, there is active academic interest (Vellvm group, Penn/Edinburgh) in machine-checked proofs of the MIR-level SSA deconstruction passes, including critical edge splitting correctness.
- **Dynamic pre-RA pipeline reordering driven by profile feedback**: Rather than a fixed pass order (TwoAddr → PHIElim → SlotIdx → LiveIntervals → Coalesce → MachineCombiner → Schedule), a profile-guided pipeline manager could reorder or repeat passes (e.g., run a second coalescing round after scheduling for hot loops), enabled by cheap re-computation of `SlotIndexes` on modern hardware.
