...
- [181.1 The Verification Landscape](#1811-the-verification-landscape)
  - [181.1.1 Two Axes: Automation and Scope](#18111-two-axes-automation-and-scope)
  - [181.1.2 The Practical Threshold in April 2026](#18112-the-practical-threshold-in-april-2026)
  - [181.1.3 Why LLVM Developers Should Care](#18113-why-llvm-developers-should-care)
- [181.2 Dafny: Push-Button Code Verification](#1812-dafny-push-button-code-verification)
  - [181.2.1 The Specification Sublanguage](#18121-the-specification-sublanguage)
  - [181.2.2 Binary Search with Loop Invariant](#18122-binary-search-with-loop-invariant)
  - [181.2.3 Generics and Trait-Style Specifications](#18123-generics-and-trait-style-specifications)
  - [181.2.4 Ghost Code and Heap Reasoning](#18124-ghost-code-and-heap-reasoning)
  - [181.2.5 Limitations and LLM Integration](#18125-limitations-and-llm-integration)
- [181.3 Verus: Dafny-Style Verification for Rust](#1813-verus-dafny-style-verification-for-rust)
  - [181.3.1 The `verus!` Macro and Function Modes](#18131-the-verus-macro-and-function-modes)
  - [181.3.2 Ghost State: `Ghost<T>` and `Tracked<T>`](#18132-ghost-state-ghostt-and-trackedt)
  - [181.3.3 The `cargo verus` Workflow](#18133-the-cargo-verus-workflow)
  - [181.3.4 AWS Deployments](#18134-aws-deployments)
  - [181.3.5 Current Maturity and Limitations](#18135-current-maturity-and-limitations)
- [181.4 F* and HACL*: Verified Cryptography in Production](#1814-f-and-hacl-verified-cryptography-in-production)
  - [181.4.1 The Effect System](#18141-the-effect-system)
  - [181.4.2 A Cryptographic Lemma in F*](#18142-a-cryptographic-lemma-in-f)
  - [181.4.3 HACL*: High-Assurance Cryptographic Library](#18143-hacl-high-assurance-cryptographic-library)
  - [181.4.4 Why3 as a Meta-Platform](#18144-why3-as-a-meta-platform)
  - [181.4.5 Jasmin: Assembly-Level Verification for Cryptography](#18145-jasmin-assembly-level-verification-for-cryptography)
- [181.5 Bounded Model Checking: CBMC and Kani](#1815-bounded-model-checking-cbmc-and-kani)
  - [181.5.1 CBMC: Bounded Model Checking for C](#18151-cbmc-bounded-model-checking-for-c)
  - [181.5.2 Kani: Bounded Model Checking for Rust](#18152-kani-bounded-model-checking-for-rust)
  - [181.5.3 Kani at AWS](#18153-kani-at-aws)
  - [181.5.4 CBMC vs. Kani](#18154-cbmc-vs-kani)
- [181.6 Temporal Logic and Model Checking](#1816-temporal-logic-and-model-checking)
  - [181.6.1 TLA+ and the TLC Model Checker](#18161-tla-and-the-tlc-model-checker)
  - [181.6.2 Apalache: Symbolic TLA+](#18162-apalache-symbolic-tla)
  - [181.6.3 SPIN and nuXmv](#18163-spin-and-nuxmv)
  - [181.6.4 SV-COMP 2026](#18164-sv-comp-2026)
- [181.7 SMT and SAT Engines](#1817-smt-and-sat-engines)
  - [181.7.1 Z3](#18171-z3)
  - [181.7.2 CVC5](#18172-cvc5)
  - [181.7.3 Bitwuzla](#18173-bitwuzla)
  - [181.7.4 Kissat](#18174-kissat)
- [181.8 Neural Network Verification](#1818-neural-network-verification)
  - [181.8.1 α,β-CROWN](#18181-crown)
  - [181.8.2 Marabou](#18182-marabou)
- [181.9 Refinement Types for Rust: Flux](#1819-refinement-types-for-rust-flux)
  - [181.9.1 The Specification Language](#18191-the-specification-language)
  - [181.9.2 The `RefinedBy` Attribute](#18192-the-refinedby-attribute)
  - [181.9.3 The Verification Architecture](#18193-the-verification-architecture)
  - [181.9.4 Comparison with Verus](#18194-comparison-with-verus)
  - [181.9.5 Current Limitations](#18195-current-limitations)
- [181.10 AI-Assisted Proof](#18110-ai-assisted-proof)
  - [181.10.1 The 2025–2026 Shift](#181101-the-20252026-shift)
  - [181.10.2 Lean Copilot and LeanDojo](#181102-lean-copilot-and-leandojo)
  - [181.10.3 Open-Weight Proof Models](#181103-open-weight-proof-models)
  - [181.10.4 The 2026 Vericoding Benchmarks](#181104-the-2026-vericoding-benchmarks)
  - [181.10.5 Practical Recommendations](#181105-practical-recommendations)
...
- **Verus async support (RFC in progress)**: The Verus team has an open design thread on supporting `async fn` verification via MIR-level desugaring to state machines. A prototype covering `tokio::select!` and single-await-point functions is expected to land, unblocking AWS Kani/Verus co-deployment on async Rust codebases (tracking: [verus-lang/verus#1187](https://github.com/verus-lang/verus/issues/1187)).
- **Kani contract-based harness generation (v0.56–v0.58)**: The `#[requires]`/`#[ensures]` contract framework (§181.5.3) is being extended to automatically synthesise `#[kani::proof_for_contract]` harnesses for public API functions, reducing the per-function harness authoring burden. The AWS s2n-tls team is dogfooding this on new TLS 1.3 extension handling code.
- **EU AI Act enforcement (August 2026)**: Annex III high-risk AI deployments under the EU AI Act formally require documented accuracy and robustness testing as of August 2026. This is driving commercial adoption of α,β-CROWN and Marabou for medical-imaging and autonomous-vehicle classifiers; expect vendor certifications citing VNN-COMP results to appear in CE conformity documentation.
- **CVC5 v1.3 with Alethe proof certificates**: CVC5's upcoming release adds Alethe proof production for the `QF_AUFBV` fragment (arrays + uninterpreted functions + bitvectors), which covers most CBMC/Kani verification conditions. This will enable independent certificate checking for bounded model checking results under DO-178C tool qualification requirements.
- **SV-COMP 2027 track proposal — MLIR/LLVM IR category**: A proposal circulating on the SV-COMP mailing list (sv-comp@sosy-lab.org) would add an LLVM IR verification track, allowing tools like SMACK (which translates LLVM IR to Boogie) and SeaHorn to be benchmarked directly on LLVM IR inputs rather than requiring the C front-end, reducing front-end noise in the results.

### 2.5-Year Horizon (Mid-Term, by ~October 2028)

- **MLIR translation-validation pipeline (Alive2-for-MLIR)**: The LLVM community is developing a translation-validation infrastructure for MLIR dialect lowering analogous to Alive2's peephole-rewrite checking. Initial scope covers the Linalg-to-Vector and Vector-to-LLVM lowering paths; the tool would verify that each rewrite preserves tensor semantics as a Z3/CVC5 formula. An RFC on discourse.llvm.org (circa Q3 2026) is expected to formalise the semantics model. This closes the HACL* trusted-base gap identified in §181.4.3 for ML inference codebases compiled through MLIR.
- **Jasmin/libjade ARM64 support**: The Jasmin compiler currently targets x86-64; ARM64 (AArch64) support is on the libjade roadmap for 2027 to enable formally verified ML-KEM and ML-DSA implementations for mobile and server-class ARM platforms (Apple M-series, AWS Graviton). The Coq-verified `jasminc` compiler backend will need a new proof of the ARM64 instruction-selection pass, representing a substantial Coq engineering effort.
- **Dafny 4.x compilation to Rust**: The Dafny team is developing a Rust extraction backend (following the existing C#, Java, Go, and Python backends). Once available, Dafny programs can be extracted to Rust and compiled with rustc, combining Dafny's push-button verification with Rust's zero-cost abstractions. This pipeline would allow Dafny-verified algorithms to run in environments where the C# or Java runtimes are unavailable (embedded, WASM, kernel modules).
- **Flux production graduation**: The UCSD Programming Systems group has signalled plans to graduate Flux from research prototype to a maintained crate with a stable API and rustc plugin stability guarantee, following the precedent set by Verus's production adoption at AWS. Key milestones: trait-contract verification (all implementations checked against the annotated trait signature) and closure support. Timeline aligns with rustc plugin stabilisation work in the Rust compiler team.
- **LLM proof models for LLVM IR correctness**: Research groups (notably at CMU and ETH Zürich) are investigating LLM-assisted generation of Vellvm-style Coq proofs for LLVM IR transformation correctness — using the Lean 4 / Coq proof kernel as a reward signal, analogous to DeepSeek-Prover-V2 on Mathlib4. The target is automatically generated proof obligations for LLVM InstCombine rewrites, complementing the SMT-based Alive2 approach (§181.7) with interactive-proof coverage of rewrites that Alive2 cannot discharge.

### 5-Year Horizon (Long-Term, by ~2031)

- **Verified MLIR dialect lowering for neural network compilation**: A full translation-validation chain from high-level MLIR (Linalg/tensor dialect) through vectorisation to LLVM IR and machine code would allow the end-to-end correctness argument for ML inference to close: the α,β-CROWN / Marabou robustness proof at the network level would compositionally extend to the compiled binary. This requires formalising the semantics of the Vector and GPU dialects, likely in Lean 4 given the active MLIR-in-Lean4 work beginning at MIT and Oxford in 2025–2026.
- **Common Criteria EAL6+ certification for Rust systems using Verus/Kani**: The CC EAL6+ certification level requires formal verification of the Security Target at the source level. As Verus matures and Kani's BMC coverage grows, the combination may satisfy the EAL6+ formal verification requirement for hypervisors and secure-boot firmware written in Rust — a path currently blocked by the lack of a Rust-native formal verification tool accepted by national certification bodies (BSI, ANSSI, NIAP). By 2031, AWS's published Verus/Kani deployments are expected to provide sufficient regulatory precedent.
- **Post-quantum cryptography full-stack verification**: By 2031, the NIST PQC transition (FIPS 203/204/206 fully deployed) will drive demand for FIPS 140-3 Level 4 validated implementations of ML-KEM, ML-DSA, and SLH-DSA. libjade's Jasmin-verified implementations and HACL*'s F*-verified implementations are the two candidates for such validation; the remaining gap is assembly-level verification for ARM64 and RISC-V SIMD extensions (SVE2, V extension) where neither libjade nor HACL* currently has coverage.
- **Fully automated program verification for competitive-programming-scale problems**: DeepSeek-Prover-V2 and its successors are on a trajectory toward 80%+ pass rates on the miniF2F and AIME benchmarks within 3–4 years, extrapolating from current RL scaling laws. For practical program verification (rather than competition mathematics), the implication is that Dafny/Verus specification generation will approach near-automation for algorithmic code of the complexity handled by competitive programming. The remaining human-in-the-loop role shifts from writing specifications to validating that the generated specifications correctly capture intent — a philosophical shift in how formal verification integrates into the software engineering workflow.
