...
- [ARM64EC and ARM64X: AArch64 on Windows](#arm64ec-and-arm64x-aarch64-on-windows)
- [AArch64 Lightweight Fault Isolation (LFI)](#aarch64-lightweight-fault-isolation-lfi)
...
## ARM64EC and ARM64X: AArch64 on Windows

Microsoft's ARM64EC (Emulation Compatible) ABI and the associated ARM64X fat-binary format address a core challenge for ARM Windows: running native AArch64 code alongside the x86-64 emulation layer that supports legacy software. Understanding ARM64EC is essential for any developer targeting Windows-on-ARM systems where native and emulated code must interoperate at the function level.

### Background: The x64 Emulation Problem

Windows on ARM (WoA) has always shipped an x86 (and now x86-64) emulator, allowing unmodified legacy applications to run. The boundary between native ARM64 code and emulated x64 code is the core problem: when an x64-emulated binary calls into a native ARM64 DLL (or vice versa), argument registers, shadow space conventions, and unwind information must be reconciled at each call site. Prior to ARM64EC, this was handled coarsely by keeping entire modules either native or emulated. ARM64EC allows mixing at the function granularity.

### ARM64EC Calling Convention

ARM64EC does not use the standard AAPCS64 calling convention. Instead, its register layout is designed to project onto the x64 shadow stack and register model:

| Role | AAPCS64 | ARM64EC |
|------|---------|---------|
| Integer arg 1 | x0 | x0 (same) |
| Integer arg 2 | x1 | x1 (same) |
| Integer arg 3 | x2 | x2 (same) |
| Integer arg 4 | x3 | x3 (same) |
| Volatile scratch | x9–x15 | x9–x15 (same, but x4–x8 constraints differ) |
| Shadow stack area | not used | 32 bytes at [sp] on entry (x64 ABI compat) |
| x18 | platform register | ABI-reserved (mirrors x64 GS base semantics) |

The 32-byte shadow space at the top of the stack mirrors the x64 shadow parameter area. ARM64EC callers allocate this space before any call, so x64-to-ARM64EC thunks can spill RCX/RDX/R8/R9 into it without additional stack manipulation.

### Entry Thunks: Bridging Native and Emulated Code

Every ARM64EC-exported function has two entry points:

1. **Native ARM64EC entry**: Standard ARM64EC calling convention, used when a native ARM64 caller invokes the function.
2. **x64-compatible thunk** (the "exit thunk"): An x64-ABI entry point generated by the MSVC or Clang toolchain. When the x64 emulator calls into a native function, it enters through this thunk. The thunk translates x64 argument registers (RCX, RDX, R8, R9) to ARM64EC registers and transfers control to the native implementation.

LLVM generates these thunks automatically when compiling for `--target=arm64ec-pc-windows-msvc`:

```bash
# Compile for ARM64EC target
clang-cl --target=arm64ec-pc-windows-msvc \
    /O2 /MD \
    foo.cpp /link /out:foo.dll
```

The resulting COFF object file contains `.text$mn` (native ARM64EC code) alongside `#exitthunk` and `#enterthunk` sections for each exported function.

### ARM64X: Fat Binaries for ARM Windows

ARM64X is Microsoft's PE/COFF fat-binary format that embeds both classic ARM64 (AAPCS64) code and ARM64EC code in a single DLL or EXE. The Windows loader selects the appropriate code based on whether the process is running native ARM64 or under the x64 compatibility layer. The PE/COFF `IMAGE_FILE_MACHINE_ARM64X` machine type signals an ARM64X binary to the loader.

LLD's COFF backend handles ARM64X emission when passed both an ARM64 and an ARM64EC object file:

```bash
# LLD merges ARM64 and ARM64EC objects into ARM64X PE
lld-link /machine:arm64x \
    /out:foo_arm64x.dll \
    foo_arm64.obj foo_arm64ec.obj \
    /dll /def:foo.def
```

The resulting binary contains merged section headers and two separate code ranges. The PE's `IMAGE_DYNAMIC_RELOCATION_TABLE` carries the structural information that distinguishes which virtual address ranges contain ARM64 vs ARM64EC code.

### Hybrid Unwind Tables

ARM64EC uses hybrid unwind tables that must satisfy both the ARM64 and x64 unwind conventions. Each ARM64EC function has:
- A standard ARM64 `.xdata` unwind record (for native ARM64 exception handling)
- An x64-compatible `RUNTIME_FUNCTION` entry with `UNWIND_INFO` (for the x64 exception dispatcher path)

LLVM's `MCObjectStreamer` for COFF emits both forms when the target is ARM64EC. The relevant source is `llvm/lib/MC/MCAsmStreamer.cpp` and the ARM64EC-specific emission in `llvm/lib/Target/AArch64/AArch64MCTargetDesc.cpp`.

### Cross-References

- For MS ABI calling convention details: [Chapter 43 — The Microsoft ABI](../part-06-clang-codegen/ch43-the-microsoft-abi.md)
- For LLD COFF backend internals: [Chapter 78 — LLD: The LLVM Linker](../part-13-lto-whole-program/ch78-lld-the-llvm-linker.md)

---

## AArch64 Lightweight Fault Isolation (LFI)

Software Fault Isolation (SFI) has long been the technique of choice for sandboxing native code in contexts where OS-level process isolation is too expensive: browser JIT engines, plugin systems, and user-space extensions. WebAssembly provides isolation through structured control flow and type-safe memory, but at the cost of a full compilation target and runtime overhead. AArch64 Lightweight Fault Isolation (LFI), introduced in LLVM 22 (2026), provides an alternative: hardware-assisted SFI that rewrites native AArch64 binary code at the MC layer to enforce sandbox boundaries with lower overhead than either full OS isolation or WebAssembly sandboxing.

### The Problem: Native Code Sandboxing

Traditional SFI approaches fall into two categories:

- **NaCl-style (Native Client)**: Requires code to be compiled from scratch for a restricted ISA subset; cannot sandbox arbitrary binaries.
- **WASM/emulation**: Full-fidelity isolation, but requires a runtime interpreter or re-compilation; overhead is 10–30% for compute-intensive code.

AArch64 LFI targets a third niche: existing native AArch64 code that needs sandbox boundaries, with overhead in the 5–15% range. The threat model addresses an honest (non-malicious) module that may have bugs — the sandbox prevents those bugs from corrupting memory or executing arbitrary code outside the module's assigned region.

### Hardware Primitives Used

LFI exploits two AArch64 hardware features:

**Branch Target Identification (BTI)**: (See §96.7) Landing-pad instructions that mark valid targets for indirect branches. A sandboxed module's indirect calls and jumps are restricted to addresses that carry BTI landing pads, preventing control-flow hijacking into the middle of instructions.

**Pointer Masking (FEAT_FPAC or address masking via bitwise AND)**: AArch64 allows the top address bits to be ignored (TBI), and load/store addresses can be masked with a bitwise AND to keep memory accesses within a bounded region. The mask corresponds to the sandbox's allocated memory range.

### MCLFIRewriter: The MC-Layer Rewriting Pass

LFI operates at the MC (Machine Code) layer, not at IR level. This is a deliberate design: it allows LFI to sandbox code that was not compiled with LFI in mind, by rewriting a completed binary. The `MCLFIRewriter` class in `llvm/lib/MC/MCLFIRewriter.cpp` performs three categories of rewrites:

**1. Load/Store Address Masking**

Every memory-accessing instruction that uses a register-computed address gets a preceding address mask:

```asm
; Before LFI rewriting:
ldr    x0, [x1]

; After LFI rewriting:
and    x1, x1, #0x0000_00FF_FFFF_FFFF   ; mask to 40-bit sandbox region
ldr    x0, [x1]
```

The mask value is the bitwise AND of the sandbox's address range limit. A 1 TB sandbox uses a 40-bit mask. The `AND` instruction uses the AArch64 immediate encoding for power-of-two-aligned masks.

**2. Indirect Branch Sandboxing**

Indirect branches (`BR`, `BLR`) are preceded by an address mask to ensure they can only target addresses within the sandbox:

```asm
; Before LFI rewriting:
blr    x8                     ; indirect call to x8

; After LFI rewriting:
and    x8, x8, #SANDBOX_MASK  ; constrain target to sandbox range
blr    x8
```

Additionally, the callee must begin with a `BTI C` landing pad (the sandbox runtime enforces BTI is enabled), so even if an attacker can influence x8, they can only branch to valid `BTI C` sites.

**3. Guarded Call Sites at Sandbox Exit**

When sandboxed code calls out to the host (a trusted function outside the sandbox), it uses a "trampoline" address provided by the sandbox runtime. The LFI rewriter inserts a check that the target of any such call matches the allowed trampoline list:

```asm
; Sandboxed indirect call via trampoline table:
ldr    x16, [x18, #trampoline_offset]   ; x18 = sandbox info block
blr    x16                               ; call through trampoline
```

### Enabling LFI

LFI is enabled via a toolchain flag:

```bash
# Compile with AArch64 LFI sandboxing
clang --target=aarch64-linux-gnu \
    -mllvm -enable-aarch64-lfi \
    -mbranch-protection=bti \
    -O2 plugin.c -shared -o plugin.so
```

Alternatively, the `MCLFIRewriter` can post-process an existing ELF:

```bash
# Rewrite an existing AArch64 ELF binary for LFI sandboxing
llvm-lfi-rewriter --sandbox-size=1TB plugin.so -o plugin_sandboxed.so
```

### Comparison with WebAssembly Isolation

| Property | WebAssembly | AArch64 LFI |
|----------|-------------|-------------|
| Code must be recompiled | Yes (to Wasm target) | No (rewrites existing binary) |
| Control flow enforcement | Structural (type-safe CFG) | BTI landing pads |
| Memory isolation | Linear memory model | Address masking |
| Typical overhead | 10–30% | 5–15% |
| Language/ABI neutrality | Wasm-specific ABI | Native AArch64 ABI |
| Debugging support | Source maps / DWARF | Full native DWARF |

### Use Cases

- **Browser JIT sandboxing**: Isolate JIT-compiled JavaScript or WebAssembly on AArch64 hosts without the overhead of full process isolation.
- **Plugin isolation**: Sandbox third-party plugins (audio, image codecs) in an application without spawning a separate process.
- **Managed runtime extensions**: Isolate native extensions in Python, Java, or .NET runtimes.

### Implementation Sources

The primary implementation lives in:
- `llvm/lib/MC/MCLFIRewriter.cpp`: The MC-layer rewriting pass
- `llvm/lib/Target/AArch64/AArch64LFI.h`: AArch64-specific mask computations
- `llvm/lib/Target/AArch64/AArch64TargetMachine.cpp`: Hooks for enabling LFI as a post-processing step

Cross-references: §96.7 (BTI) and §96.6 (PAuth, which works synergistically with LFI for return-address integrity).
